...
const createPDFTemplate = async (): Promise<Template> => {
  ...
  const contractBg1 = await getBase64FromUrl('/bgc1.jpg').catch(() => '');
  const contractBg2 = await getBase64FromUrl('/bgc2.jpg').catch(() => '');
...
export async function generateInvoicePDF(data: InvoiceData): Promise<Uint8Array> {
  try {
    ...
    throw new Error('Failed to create PDF file: ' + (error as Error).message);
  }
}
...
export async function generateContractPDF(contractData: ContractData, additionalData?: Partial<InvoiceData>): Promise<Uint8Array> {
  try {
    ...
    throw new Error('Failed to create contract PDF: ' + (error as Error).message);
  }
}
